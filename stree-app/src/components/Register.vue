<template>
    <div class="register-container">
      <div class="register-form">
        <img alt="Logo" src="../assets/arrow2.png" class="logo">
        ...
        <form @submit.prevent="register">
          ...
          <div class="form-group">
            <div class="input-line">
              <input type="tel" v-model="mobileNumber" placeholder="Mobile Number" class="form-control" pattern="\d*" inputmode="numeric">
          ...
          <div class="form-group">
            ...
            <div class="password-strength">
              <div v-for="index in 4" :key="index" :class="{ active: index <= passwordStrength || (index === 1 && password.length >= 9) }"></div>
            </div>
            <div v-if="passwordError" class="error-message">{{ passwordError }}</div>
          </div>
          <div class="form-group">
            <div class="input-line">
              <input style="margin-bottom: -20px;" type="password" v-model="confirmPassword" placeholder="Confirm Password" class="form-control">
              <div v-if="confirmPasswordError" class="error-message">{{ confirmPasswordError }}</div>
            </div>
          </div>
          <div class="form-group">
            <button type="submit" class="btn btn-danger" :class="{ 'disabled': !isValidInput }" :disabled="!isValidInput">Register</button>
          </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        passwordStrength: 0, // Added property to store password strength
        passwordError: '', // Added property to store password error message
      };
    },
    computed: {
      isValidInput() {
        return (
          ...
          !this.passwordError // Added condition for password error
        );
      },
    },
    methods: {
      ...
      this.fullNameError = 'Full Name should not contain words.';
    } else {
      this.fullNameError = '';
    }
  },
    ...
    } else if (/[a-zA-Z]/.test(this.mobileNumber)) {
      this.mobileNumberError = 'Mobile Number should not contain words.';
    } else if (!this.isValidMobileNumber(this.mobileNumber)) {
  ...
  .register-form {
    ...
    height: 600px;
    margin-bottom: 30px;
    overflow-y: auto; /* Added overflow-y property for scrollable feature */
  }
  ...
  .error-message {
    color: red;
    font-size: 12px;
    height: 16px; /* Added fixed height for error message container */
  }
